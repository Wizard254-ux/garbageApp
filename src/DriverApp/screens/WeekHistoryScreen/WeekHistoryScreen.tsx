...
export const WeekHistoryScreen: React.FC = () => {
  ...
  const fetchWeekPickups = async (useCache = true) => {
    const weekStart = getWeekStart();
    const cacheKey = `week_pickups_${weekStart}`;
    ...
    try {
      ...
      console.error('Failed to fetch week pickups:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    await fetchWeekPickups(false); // Force fresh data on refresh
    setRefreshing(false);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.title}>This Week History</Text>
        <Text style={styles.subtitle}>{getWeekRange()}</Text>
      </View>

      <View style={styles.filterContainer}>
        {(['all', 'picked', 'unpicked', 'missed'] as FilterType[]).map((filterType) => (
          <TouchableOpacity
            key={filterType}
            style={[
              styles.filterButton,
      ...
      {showRouteDropdown && (
        <Modal visible={showRouteDropdown} transparent animationType="none">
          <TouchableOpacity 
      ...
      {loading ? (
        <View style={[styles.loadingContainer, { backgroundColor: colors.background }]}>
          <Text style={[styles.loadingText, { color: colors.primary }]}>Loading this week's pickups...</Text>
        </View>
      ) : (
        <FlatList
          data={pickups}
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
                No pickups found for this week
              </Text>
            </View>
          }
        />
