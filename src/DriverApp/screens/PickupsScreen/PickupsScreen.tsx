...
type PickupTab = 'pending' | 'completed' | 'bags';
...
export const PickupsScreen: React.FC = () => {
  ...
  const loadPickups = async () => {
    if (activeTab === 'bags') {
      // For bags tab, load eligible clients instead of pickups
      try {
        const response = await apiService.getEligibleClientsForBags(selectedRouteId || undefined);
        console.log('yoooooo;',response.data.data)
        if (response?.data?.data) {
          // Transform to match expected format and filter only active clients
          const transformedClients = response.data.data
            .filter((client: any) => {
              console.log(`Bag client ${client.name}: isActive = ${client.isActive}`);
              return client.isActive === true; // Only explicitly active clients
            })
            .map((client: any) => ({
              id: client.id,
              userId: client.id, // For bag distribution, use client ID directly
              name: client.name,
              email: client.email,
              phone: client.phone,
              ...
              routeName: client.route?.name || 'No Route',
              isActive: client.isActive
            }));
          setUsers(transformedClients);
        }
      } catch (error) {
        console.error('Failed to load eligible clients:', error);
      }
      return;
    }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.tabContainer}>
          ...
          <TouchableOpacity
            style={[styles.tab, activeTab === 'bags' && styles.activeTab]}
            onPress={() => setActiveTab('bags')}
          >
            <Text style={[
              styles.tabText,
              { color: activeTab === 'bags' ? colors.primary : colors.textSecondary }
            ]}>
              Bags
            </Text>
          </TouchableOpacity>
        ...
        {activeTab === 'bags' ? (
          <View style={styles.bagsContainer}>
            <View style={styles.bagsHeader}>
              <View>
                <Text style={[styles.bagsTitle, { color: colors.text }]}>Bag Distribution</Text>
                <Text style={[styles.bagsSubtitle, { color: colors.textSecondary }]}>
                  {!selectedClient ? 'Select a client to distribute bags to' : 'Enter distribution details'}
                </Text>
              </View>
              <View style={styles.bagsHeaderActions}>
                <TouchableOpacity
                  style={[styles.actionButton, { backgroundColor: colors.primary }]}
                  onPress={() => setShowBagHistory(true)}
                >
                  <Ionicons name="time-outline" size={16} color="white" />
                  <Text style={[styles.actionButtonText, { fontSize: 12 }]}>History</Text>
                </TouchableOpacity>
                {!selectedClient && (
                  <TouchableOpacity
                    style={[styles.reloadButton, { backgroundColor: colors.surface, borderColor: colors.border }]}
                    onPress={async () => {
                      try {
                        // For bags tab, load eligible clients instead of pickups
                        const response = await apiService.getEligibleClientsForBags(selectedRouteId || undefined);
                                                  console.log('settign users ',response.data.data)

                        if (response?.data?.data) {
                          // Transform to match expected format
                          const transformedClients = response.data.data.map((client: any) => ({
                            id: client.id,
                            userId: client.id, // For bag distribution, use client ID directly
                            name: client.name,
                            email: client.email,
                        ...
                        console.error('Failed to reload eligible clients:', error);
                        Alert.alert('Error', 'Failed to load eligible clients');
                      }
                    }}
                  >
            ...
            {!selectedClient ? (
                  ...
                  renderItem={({ item }) => (
                    ...
                    >
                      <View style={styles.clientCardContent}>
                        ...
                        <Text style={[styles.clientDetails, { color: colors.textSecondary }]}>{item.email}</Text>
                        <Text style={[styles.clientDetails, { color: colors.textSecondary }]}>{item.phone}</Text>
                        <Text style={[styles.clientDetails, { color: colors.textSecondary }]}>{item.address}</Text>
                      </View>
                  ...
                  ListEmptyComponent={
                    <View style={styles.emptyContainer}>
                      ...
                      <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
                        No clients available for bag distribution
                      </Text>
                      <Text style={[styles.emptySubtext, { color: colors.textSecondary }]}>
                        {selectedRouteId ? 'Try selecting a different route' : 'Select a route to see clients'}
                      </Text>
                    </View>
                  }
                />
              ...
              <View>
                <View style={[styles.selectedClientCard, { backgroundColor: colors.surface, borderColor: colors.primary }]}>
                  <View style={styles.selectedClientInfo}>
                    <Text style={[styles.selectedClientName, { color: colors.text }]}>{selectedClient.name}</Text>
                    <Text style={[styles.selectedClientDetails, { color: colors.textSecondary }]}>{selectedClient.email}</Text>
                    <Text style={[styles.selectedClientDetails, { color: colors.textSecondary }]}>{selectedClient.address}</Text>
                  </View>
                  <TouchableOpacity
                    style={[styles.changeClientButton, { backgroundColor: colors.cardBackground }]}
                    onPress={() => {
                      console.log('slected client ',selectedClient)
                      setSelectedClient(null);
                      setRecipientEmail('');
                      setNumberOfBags('');
                    }}
                  >
                    <Text style={[styles.changeClientText, { color: colors.primary }]}>Change</Text>
                  </TouchableOpacity>
                </View>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    {
                      backgroundColor: (!recipientEmail || !numberOfBags || loading) ? colors.textSecondary : colors.primary,
                      opacity: (!recipientEmail || !numberOfBags || loading) ? 0.5 : 1
                    }
                  ]}
                  onPress={() => handleDistributeBags(selectedClient.userId || selectedClient.id, selectedClient.name)}
                  disabled={!recipientEmail || !numberOfBags || loading}
                >
                  {loading ? (
                    <ActivityIndicator size="small" color="white" />
                  ) : (
                    <Text style={styles.distributeButtonText}>Send Distribution Code</Text>
                  )}
                </TouchableOpacity>
              </View>
            ) : (
              ...
              <View>
                ...
                <Text style={[styles.verificationText, { color: colors.text }]}>
                  Enter the verification code sent to {recipientEmail}
                </Text>
                ...
                <TouchableOpacity
                  style={[
                    styles.verifyButton, 
                    { 
                      backgroundColor: verificationLoading ? colors.textSecondary : colors.primary,
                      opacity: verificationLoading ? 0.7 : 1
                    }
                  ]}
                  onPress={handleVerifyDistribution}
                  disabled={verificationLoading}
                >
                  {verificationLoading ? (
                    <ActivityIndicator size="small" color="white" />
                  ) : (
                    <Text style={styles.verifyButtonText}>Verify Distribution</Text>
                  )}
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    { 
                      ...
                      opacity: verificationLoading ? 0.5 : 1
                    }
                  ]}
                  onPress={() => {
                    setDistributionId('');
                  ...
                  disabled={verificationLoading}
                >
                  <Text style={[styles.backButtonText, { color: colors.text }]}>Back to Details</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
